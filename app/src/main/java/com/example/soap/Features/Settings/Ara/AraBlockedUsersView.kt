...
@Composable
fun AraBlockedUsersView(
    initialBlockedUsers: List<String>,
    ...
    Scaffold(
        topBar = {
            SettingsViewNavigationBar(
                title = "Blocked Users",
                onDismiss = { navController.navigate(Channel.AraSettings.name )}
            )
        }
    ) { innerPadding ->
        LazyColumn(
...
private fun Preview(){
    Theme {
        AraBlockedUsersView(listOf("Nickname 1", "Nickname 2"), rememberNavController())
    }
}