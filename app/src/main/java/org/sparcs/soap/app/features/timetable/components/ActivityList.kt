...
fun ActivityList(
    ...
    ElevatedCard(
        ...
        Column(
            ...
            Text(
                pluralStringResource(R.plurals.activities_count, activities.size, activities.size),
            ...
            if (sorted.isEmpty()) {
                Box(
                    ...
                    Text(
                        stringResource(R.string.activity_list_empty),
                    ...
                    Row(
                        Modifier
                            ...
                            .combinedClickable(
                                ...
                                onLongClick = {
                                    haptic.performHapticFeedback(HapticFeedbackType.LongPress); selected =
                                    activity; showActions = true
                                }
                            )
                        ...
                        Column(
                            ...
                            val day = DayType.fromValue(activity.day)
                                ?.let { stringResource(it.stringValue) }.orEmpty()
                            Text(
                                "$day · ${activityTime(activity.begin)} – ${activityTime(activity.end)}",
                                style = MaterialTheme.typography.bodySmall,
                                color = MaterialTheme.colorScheme.onSurfaceVariant
                            )
                        }
                        IconButton(onClick = { selected = activity; showActions = true }) {
                            Icon(
                                ...
                                stringResource(R.string.activity_options, activity.title)
                            )
                        }
                    }
...
private fun Preview(){
    Theme {
        ActivityList(
            activities = listOf(
                TimetableActivity(1, "Study group", "Library", 0, 540, 600),
                TimetableActivity(2, "Meeting", "Room 101", 1, 600, 660),
                TimetableActivity(3, "Workshop", "Hall A", 2, 660, 720)
            ),
